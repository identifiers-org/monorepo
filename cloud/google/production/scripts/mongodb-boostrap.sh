...
function launch_stateful_set() {
    ...
    while [ "`kubectl get statefulset/mongod -o yaml | yq .status.readyReplicas`" != "${MONGODB_BOOTSTRAP_N_REPLICAS}" ]; do
        tlog info "[CLOUD] Only #`kubectl get statefulset/mongod -o yaml | yq .status.readyReplicas` out of #${MONGODB_BOOTSTRAP_N_REPLICAS} are up, waiting..."
        sleep 3
    done
    sleep 18
